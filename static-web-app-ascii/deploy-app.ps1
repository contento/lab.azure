...
Write-Host " Starting Full Application Deployment     " -ForegroundColor Cyan
Write-Host "==========================================" -ForegroundColor Cyan

& (Join-Path $PSScriptRoot "infra/deploy-infrastructure.ps1") @PSBoundParameters
if ($LASTEXITCODE -ne 0) { throw "Infrastructure deployment failed." }

& (Join-Path $PSScriptRoot "deploy-code.ps1") -BaseName $BaseName -Environment $Environment -Location $Location -LocationCode $LocationCode -SubscriptionId $SubscriptionId
