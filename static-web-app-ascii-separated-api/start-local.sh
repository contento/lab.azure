...
swa_port="${SWA_PORT:-4281}"
api_port="${API_PORT:-7072}"
...
printf 'window.TYPECAST_API_BASE_URL = "http://127.0.0.1:%s";\n' "$api_port" > "$project_root/app/config.js"
...
npx --yes --package @azure/static-web-apps-cli swa start "$project_root/app" --api-devserver-url "http://127.0.0.1:$api_port" --swa-config-location "$project_root/app" --port "$swa_port"
